...
tests = testGroup "SHA256 Scramble"
    ...
    , testCase "golden: salt=12345678901234567890 pass=password" $ do
        result <- scrambleSHA256 "12345678901234567890" "password"
        let expected = either error id $ B16.decode "718d80fb92b693f7ee1fcad95bfe7a2be1f332ac2c3ec6cdc85893b65b240650"
        assertEqual "golden vector 1" expected result

    , testCase "golden: salt=ABCDEFGHIJKLMNOPQRST pass=secret" $ do
        result <- scrambleSHA256 "ABCDEFGHIJKLMNOPQRST" "secret"
        let expected = either error id $ B16.decode "d721e183c1f036a196c1389201b8d53f38064a16f1d0923683ab886763152d75"
