...
tests = testGroup "SHA1 Scramble"
    ...
    , testCase "golden: salt=12345678901234567890 pass=password" $ do
        result <- scrambleSHA1 "12345678901234567890" "password"
        let expected = either error id $ B16.decode "1957dce2724282e018f40d905824cb6361f88d41"
        assertEqual "golden vector 1" expected result

    , testCase "golden: salt=ABCDEFGHIJKLMNOPQRST pass=secret" $ do
        result <- scrambleSHA1 "ABCDEFGHIJKLMNOPQRST" "secret"
        let expected = either error id $ B16.decode "28441590674285e7d03cae7af237504797f70e91"
